...
extension ContentView {
    @_dynamicReplacement(for: body) private var __preview__body: some View {
            ...
            if authService.isAuthenticated {
                TabView(selection: $selectedTab) {
                    HomeView(authService: authService, apiService: apiSerive)
                        .navigationBarHidden(__designTimeBoolean("#3801.[1].[4].property.[0].[0].arg[0].value.[0].[0].[0].arg[1].value.[0].modifier[0].arg[0].value", fallback: true))
                        .tabItem {
                    ...
                    NavigationView {
                        Text(__designTimeString("#3801.[1].[4].property.[0].[0].arg[0].value.[0].[0].[0].arg[1].value.[2].arg[0].value.[0].arg[0].value", fallback: "Teams view"))
                    }
                    .tabItem {
                        Image(__designTimeString("#3801.[1].[4].property.[0].[0].arg[0].value.[0].[0].[0].arg[1].value.[2].modifier[0].arg[0].value.[0].arg[0].value", fallback: "Teams"))
                    }
                    .tag(Tab.teams)

                    SearchView()
                    .tabItem {
                        Image(__designTimeString("#3801.[1].[4].property.[0].[0].arg[0].value.[0].[0].[0].arg[1].value.[3].modifier[0].arg[0].value.[0].arg[0].value", fallback: "Search"))
                    }
                    .tag(Tab.search)
                    
                    ProfileView(authService: authService)
                    .tabItem {
                        Image(__designTimeString("#3801.[1].[4].property.[0].[0].arg[0].value.[0].[0].[0].arg[1].value.[4].modifier[0].arg[0].value.[0].arg[0].value", fallback: "Profile"))
                    }
                    .tag(Tab.profile)
                }
                .accentColor(.red)
        ...
        .onChange(of: authService.isAuthenticated) { newValue, oldValue in
            authService.isLoading = __designTimeBoolean("#3801.[1].[4].property.[0].[0].modifier[0].arg[1].value.[0].[0]", fallback: false)
        }
        .onAppear {
