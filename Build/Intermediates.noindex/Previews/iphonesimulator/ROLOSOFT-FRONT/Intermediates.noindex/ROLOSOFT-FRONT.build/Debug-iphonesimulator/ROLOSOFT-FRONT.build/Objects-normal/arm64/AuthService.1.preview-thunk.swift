...
extension AuthService {
    @_dynamicReplacement(for: logIn(username:password:completion:)) private func __preview__logIn(username: String, password: String, completion: @escaping (User) -> Void) {
        ...
        print(__designTimeString("#2697.[1].[2].[0].arg[0].value", fallback: "HOLA"))
        guard let baseURL = baseURL else {
            print("Invalid base URL")
            return
        }
        
        let url = baseURL.appendingPathComponent(__designTimeString("#2697.[1].[2].[2].value.modifier[0].arg[0].value", fallback: "/users/login"))
        
        let body: [String: Any] = [
            __designTimeString("#2697.[1].[2].[3].value.[0].key", fallback: "email"): username,
            __designTimeString("#2697.[1].[2].[3].value.[1].key", fallback: "password"): password
        ]
        ...
        request.httpMethod = __designTimeString("#2697.[1].[2].[6].[0]", fallback: "POST")
        request.setValue(__designTimeString("#2697.[1].[2].[7].modifier[0].arg[0].value", fallback: "application/json"), forHTTPHeaderField: __designTimeString("#2697.[1].[2].[7].modifier[0].arg[1].value", fallback: "Content-Type"))
